...
export default class MandlebrotSet {
  ...
  iterate(sketch) {
    ...
    for (let index = 0; index < this.maxIndex; index++) {
      ...
      let brightness = Math.sqrt(this.nValues[index] / this.iterations) * 255
      if (this.nValues[index] === this.iterations) brightness = 120
      const color = sketch.color(this.hue, this.sat, brightness)
